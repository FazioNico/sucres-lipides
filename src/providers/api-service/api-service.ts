...
export class ApiService {
      ...
      getData(value){
        ...
        return this.http.get(queryUrl)
          ...
          .map(products => products.filter(item =>item.image_thumb_url))
          .map(products => products.filter(item =>item.categories_hierarchy))
          .map(products => products.filter(item =>item.nutriments.sugars_100g))
          .map(products => products.filter(item =>item.nutriments.carbohydrates_100g))
          .map(products => products.filter(item =>item.nutriments.fat_100g))
          .map( res => {
            //console.log(res);
            return res
          })
